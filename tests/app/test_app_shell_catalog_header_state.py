...
class AppShellCatalogHeaderStateTests(AppShellTestCase):
    ...
    def _visual_header_labels(self) -> list[str]:
        ...
        if model is None:
            return []
        return [
            str(model.headerData(logical_index, Qt.Horizontal) or "")
            ...
            if str(model.headerData(logical_index, Qt.Horizontal) or "")
        ]
    ...
    def test_header_reorder_hide_show_and_columns_movable_persist_across_restart(self):
        ...
        header = self.window.table.horizontalHeader()
        artist_column = self._catalog_column_for_header("Artist Name")
        genre_column = self._catalog_column_for_header("Genre")
        ...
        self.assertTrue(self.window.table.isColumnHidden(self._catalog_column_for_header("Genre")))
    ...
    def test_key_based_restore_prefers_column_keys_over_conflicting_legacy_payloads(self):
        ...
        header = self.window.table.horizontalHeader()
        mix_notes_column = self._catalog_column_for_header("Mix Notes")
        session_notes_column = self._catalog_column_for_header("Session Notes")
        track_title_column = self._catalog_column_for_header("Track Title")
        ...
        legacy_label_order = [
            str(self.window.table.model().headerData(logical_index, Qt.Horizontal) or "")
            for logical_index in range(self.window.table.model().columnCount())
            if str(self.window.table.model().headerData(logical_index, Qt.Horizontal) or "")
        ]
        ...
        try:
            ...
            header.moveSection(
                ...
                self.window._catalog_view_column_count() - 1,
            )
        finally:
        ...
        self.assertTrue(
            self.window.table.isColumnHidden(self._catalog_column_for_header("Session Notes"))
        )
