# Codex Prompt — Execute Phase B2 Only
...
Your task is to execute **B2 only** from that plan.

## Scope

- route live selection helpers, double-click routing, context-menu targeting, and selected-vs-visible export semantics through `CatalogTableController`
- keep current widget backend (`QTableWidget`) in place
- mark only the monolith functions replaced by this batch

## Allowed

- live controller cutover on current widget
- thin wrappers in `App` delegating to controller
- monolith deprecation markers for replaced interaction functions after successful cutover
...
- no header-state changes beyond what B1 already did
- no zoom cutover
...
- interaction/selection/export call sites
...
Write a detailed handoff document after completing B2.
...
2. Confirm B2 scope from the plan
...
- `B2 — completed — 18-apr-2026 14:32`
- `Exceptions: none`

or, if exceptions occurred:

- `B2 — completed with exceptions — 18-apr-2026 14:32`
- `Exceptions: B2.1, B2.2`
...
- `B2.1 — Phase B2 — one legacy selection helper required a temporary wrapper for compatibility — resolved`
- `B2.2 — Phase B2 — one context-menu edge case was identified during validation and deferred to B3 because it depends on model/proxy cutover — open`
...
2. confirmation that scope remained B2-only
3. QA/QC summary
...
### Final instruction
Execute only **B2** from the engineering plan.  
Do not widen scope.  
