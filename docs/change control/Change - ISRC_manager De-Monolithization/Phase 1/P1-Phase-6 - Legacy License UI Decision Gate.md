# Codex Prompt — Execute Phase 6 Only
...
Execute **Plan 1 / Phase 6 — Legacy License UI Decision Gate** only.

## Scope
- audit `LicenseUploadDialog`, `LicensesBrowserPanel`, `LicensesBrowserDialog`, `LicenseeManagerDialog`, and `_CatalogLicenseesPane`
- use current shell tests asserting the legacy license browser is not exposed as audit evidence
- audit runtime call paths, tests, documentation/examples, root compatibility imports, command/action registries, menu/ribbon/workspace registrations, string-based dynamic lookups, persisted layout/action references, database migration references if applicable, and external script/tool references inside the repository
- delete if dead
- quarantine to `isrc_manager/licenses/dialogs.py` only if a real compatibility need exists

## Allowed
- call-path audit
- test/reference audit
- documentation/example audit
- command/action/menu/ribbon/workspace registry audit
- string lookup and persisted layout/action reference audit
- database migration and external repository script/tool reference audit where applicable
- dead-code deletion
- legacy quarantine relocation if justified
- focused compile/import validation
- preservation of hidden/unexposed behavior for legacy license browser actions

## Not allowed
- no speculative preservation without real usage
- no deletion until the full required audit confirms no live or compatibility need
- no re-exposure of legacy license browser/workspace actions
...
## Handoff filename
`P1 Phase 6 handoff.md`

## Execution order
1. Read the engineering plan
2. Confirm Phase 6 scope
3. Audit only the targeted legacy license UI
4. Execute Phase 6 only
