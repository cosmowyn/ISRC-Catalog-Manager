# Codex Prompt — Execute Phase 1 Only
...
Execute **Plan 1 / Phase 1 — Logging and Prompt Helpers** only.

## Scope
- move `_JsonLogFormatter`
- move any clearly standalone prompt/helper code discovered during the batch if it is truly leaf-level and low-risk
- establish stable imports for the moved helper surface

## Allowed
- extraction of `_JsonLogFormatter`
- creation or update of `isrc_manager/app_logging.py`
- creation or update of a small prompt/helper module if justified by actual live code
- compatibility imports/re-exports if needed
- focused tests and compile/import validation

## Not allowed
- no dialog extraction
- no settings extraction
- no waveform/media extraction
- no broad helper sweep outside what is directly justified in this phase
- no Phase 2+ work
...
## Handoff filename
`P1 Phase 1 handoff.md`

## Execution order
1. Read the engineering plan
2. Confirm Phase 1 scope
3. Inspect logging/helper call sites only as needed
