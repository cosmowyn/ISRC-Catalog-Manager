# Codex Prompt — Execute Phase 8 Only
...
Execute **Plan 1 / Phase 8 — Settings Dialog Internal Health Pass** only.

## Scope
- reduce the risk of a new mini-monolith after extraction
- account for expanded theme builder, storage budget, app sound, and QSS autocomplete settings now wired into the dialog
- split internal tabs/panels/controllers only if still justified after the whole-dialog move
- improve module health without changing user-visible behavior

## Allowed
- split UI tabs/panels
- reduce local dialog module size
- isolate UI-only helpers
- internal decomposition of `isrc_manager/application_settings_dialog.py`
- creation of narrowly scoped support modules if justified
- focused settings/theme/GS1/QSS autocomplete/app-sound validation

## Not allowed
- no broad settings architecture redesign
- no permanent settings workflow ownership
- no long-lived settings/theme/history/app-sound workflow controllers
- no controller extraction that belongs to Plan 2 Phase 16
- no App controller decomposition
- no unrelated dialog extractions
- no Phase 9+ work
...
## Handoff filename
`P1 Phase 8 handoff.md`

## Execution order
1. Read the engineering plan
2. Confirm Phase 8 scope
3. Inspect the extracted settings dialog only as needed
4. Execute Phase 8 only
