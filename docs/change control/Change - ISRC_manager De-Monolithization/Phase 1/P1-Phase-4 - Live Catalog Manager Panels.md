# Codex Prompt — Execute Phase 4 Only
...
Execute **Plan 1 / Phase 4 — Live Catalog Manager Panels** only.

## Scope
- move live catalog manager pane/panel classes that are actually in use
- keep tightly coupled pane sets together where that improves cohesion
- preserve catalog manager behavior
- decide whether `CatalogManagersPanel` remains a live workspace dock implementation, is folded into diagnostics cleanup, or is better left for Phase 5 deletion

## Allowed
- extraction into `isrc_manager/catalog_managers.py`
- compatibility imports/re-exports if needed
- focused catalog manager tests
- narrowly scoped host seams if truly required
- audit of current `open_catalog_managers_dialog()` redirect and dock factory call sites

## Not allowed
- no dead catalog dialog deletion decision yet
- no legacy license UI work
- no settings/dialog/editor/media preview work beyond directly required imports
- no Phase 5+ work
...
## Handoff filename
`P1 Phase 4 handoff.md`

## Execution order
1. Read the engineering plan
2. Confirm Phase 4 scope
3. Inspect live catalog manager code only as needed
4. Execute Phase 4 only
