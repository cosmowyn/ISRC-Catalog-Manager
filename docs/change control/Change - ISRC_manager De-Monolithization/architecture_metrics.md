...
- tests still using root imports: `tests/test_history_budget_hooks.py`, `tests/test_qss_autocomplete.py`, `tests/test_shortcut_ordering.py`, `tests/test_update_ui_integration.py`, `tests/test_app_bootstrap.py`, `tests/test_migration_integration.py`, `tests/test_theme_builder.py`, `tests/app/_app_shell_support.py`
- notes / exceptions: Theme load/apply/save logic now lives in `isrc_manager.theme_builder`; settings current/apply/import/export logic now lives in `isrc_manager.settings_controller`; history retention and storage-budget orchestration now lives in `isrc_manager.history_retention_controller`; app sound/startup sound orchestration now lives in `isrc_manager.app_sound_controller`. New module sizes are 206 LOC, 400 LOC, 445 LOC, and 858 LOC respectively, all below the warning threshold. Import-cycle component count did not increase.
...
- package parity status: unchanged and valid; 27 pyproject packages match 27 filesystem packages
- tests still using root imports: `tests/test_history_budget_hooks.py`, `tests/test_qss_autocomplete.py`, `tests/test_shortcut_ordering.py`, `tests/test_update_ui_integration.py`, `tests/test_app_bootstrap.py`, `tests/test_migration_integration.py`, `tests/test_theme_builder.py`, `tests/app/_app_shell_support.py`
- notes / exceptions: Waveform cache worker and queue orchestration now live in `isrc_manager.media.waveform_cache_worker`; media player and preview-opening orchestration now lives in `isrc_manager.media.player_controller`; media and catalog audio export orchestration now lives in `isrc_manager.media.export_controller`. Existing visualization, preview/preload, equalizer player, equalizer dialog/settings, bookmark, and waveform cache infrastructure remains reused in the established media modules. Import-cycle component count did not increase.
