# Codex Prompt — Execute Phase 20 Only
...
Execute **Plan 2 / Phase 20 — Lean App Move** only.

## Scope
- move the lean `App` to `isrc_manager/main_window.py`
- make `ISRC_manager.py` import `App` and expose `main()`
- keep or split `main_window_shell.py` according to the Phase 17 end state
- preserve startup behavior and entrypoint compatibility

## Allowed
- creation or update of `isrc_manager/main_window.py`
- bounded root-file shrink of `ISRC_manager.py`
- stable entrypoint/compatibility wiring
- focused startup/shell validation

## Not allowed
- no final compatibility cleanup yet
- no unrelated behavior work
- no Phase 21 work
...
## Handoff filename
`P2 Phase 20 handoff.md`

## Execution order
1. Read the engineering plan
2. Confirm Phase 20 scope
3. Inspect `App` and entrypoint wiring only as needed
4. Execute Phase 20 only
