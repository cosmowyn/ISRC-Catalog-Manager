...
HELP_CHAPTERS: tuple[HelpChapter, ...] = (
    ...
    HelpChapter(
        ...
        <ul>
          ...
          <li><strong>Remove…</strong>: open a removal dialog, choose the profile to delete from the dropdown, then confirm the final prompt. The app captures a protected history snapshot before removing the database, so Undo can restore it while that history entry and its snapshot are retained.</li>
        </ul>
        <p>When you browse to an older unencrypted SQLite profile, the app warns before opening it and offers a migration path. You can encrypt the profile now, open it unencrypted for the session, or cancel. If you choose to keep one known unencrypted profile unencrypted, the dialog checkbox suppresses future warnings only when you open that profile. Application Settings also includes a separate global switch to turn off all unencrypted-profile warnings at your own caution. Migration creates a SQLCipher copy, preserves the original schema version for the encrypted copy, verifies the encrypted result, and replaces the active database only after the encrypted copy is valid. Invalid legacy ISRC values that cannot satisfy current validation are cleared during migration and noted in the affected track comments instead of blocking encryption. The previous unencrypted file is retained as a recovery backup in the app backups folder so the operation can fail safely.</p>
        <p>Profile-specific catalog data stays with the profile. Shared app-level conveniences such as saved themes and remembered layout settings stay available across profiles. When switching to an encrypted profile, the password prompt unlocks and prepares the target database before the active workspace is retargeted; cancelling the password prompt leaves the current profile open. If the last-used startup profile cannot be prepared or opened safely, the app leaves that database untouched, opens a fallback or startup recovery profile, and shows the original path and failure reason so you can browse, restore, or report the damaged profile from a running workspace. SQLCipher secure-memory locking is an optional deployment hardening and is not enabled during normal profile opening, because some Windows environments reject the required memory-lock request during profile switching; this does not change database-at-rest encryption. On first launch, the app can also offer to open <strong>Application Settings</strong> so you can configure registration values, database-password posture, snapshot posture, and appearance before deeper catalog work begins.</p>
    ...
    HelpChapter(
        ...
        <ul>
          ...
          <li><strong>SQLite concurrency</strong>: the app enables foreign-key enforcement and a busy timeout. Unencrypted profiles use WAL mode, while encrypted SQLCipher profiles use rollback-journal mode to avoid fragile encrypted WAL sidecars during profile switching and restart. Write-heavy jobs are serialized per profile so concurrent background writers do not fight each other.</li>
          <li><strong>Progress and cancellation</strong>: longer jobs show a progress dialog or in-place status text where practical. Undo and Redo use a non-cancellable progress dialog because replay must either finish or roll back as one protected operation; the bar advances only when a real validation, replay, verification, or interface-refresh phase completes. Some file-based jobs, such as catalog-audio copy export, can be cancelled safely.</li>
          <li><strong>Responsive history replay</strong>: catalog/profile-database Undo and Redo run on a worker-thread database connection. The catalog remains able to repaint and process window events while replay is active, but another Undo, Redo, profile switch, or database write cannot overlap the protected replay.</li>
    ...
    HelpChapter(
        ...
        <ul>
          ...
          <li><strong>Recoverable deletion</strong>: deleting an asset from the registry is recorded in Undo History. Undo restores the asset record and its app-managed file, and Redo removes them again. Files stored as external references are never deleted from disk.</li>
    ...
    HelpChapter(
        ...
        <ul>
          <li><strong>General</strong>: current profile context, database password controls, macOS stored-credential recovery under <strong>Security</strong>, an optional custom window title override, app icon, core registration details, automatic snapshots, retention and safety level, automatic cleanup, and history storage budget controls. Use Smart Budget can fill a practical history budget from the combined size of all profile databases, retained snapshot count, and safety margin.</li>
        ...
        <p>The <strong>Remember database password on this device</strong> option is off by default. When enabled, the database password is stored only in the operating-system keychain/keyring and reused for up to 30 days; it is never written to normal settings, the profile database, JSON files, environment variables, or source files. The <strong>Do not warn when opening unencrypted profiles</strong> option is also off by default and suppresses every unencrypted-profile warning only after you accept the safety warning. Use <strong>Change Password…</strong> to rekey the current SQLCipher profile after entering the current password and confirming the new one. Encrypted profile-history snapshots required by Undo and Redo are rekeyed in the same fail-safe operation; if any referenced artifact cannot be updated, the previous password remains in effect. Independent manual backups retain the password with which each backup was created.</p>
        <h3>Reset stored credentials after a macOS update</h3>
        <p>If a macOS app replacement can no longer open credentials created by an earlier build, use <strong>Settings &gt; Application Settings &gt; General &gt; Security &gt; Reset Stored Credentials…</strong>. This is an immediate, manual recovery action: the app never runs it automatically after an update. Its confirmation opens with <strong>Cancel</strong> as the safe default.</p>
        <p>The reset removes only generic-password entries owned by this app under the exact Keychain services <code>isrc-catalog-manager.database</code> and <code>isrc-catalog-manager.soundcloud</code>. That forgets remembered database passwords plus locally stored SoundCloud OAuth tokens and client secrets. It does not change the encryption password of any profile, and it does not delete or modify catalogs, profile databases, normal application settings, or unrelated Keychain passwords. Local removal also does not revoke a SoundCloud token at SoundCloud.</p>
        <p>Confirm only when you are ready to re-enter database passwords and reconnect SoundCloud. Once confirmed, closing Application Settings with <strong>Cancel</strong> cannot undo the reset. Restart the app afterward to begin a fresh session; then enter each encrypted profile's existing password and reconnect SoundCloud as needed. You do not need to inspect or clear any other part of the macOS Keychain.</p>
    ...
    HelpChapter(
        ...
        content_html="""
        <p>The app includes a persistent history system because catalog work should be recoverable. That matters especially for destructive actions, imports, large edits, settings changes, migrations, and file-backed actions.</p>
        <ul>
          <li><strong>Undo / Redo</strong>: revert or reapply the latest reversible action, including profile creation or removal, deletes of tracks, releases, works, contracts, parties, rights, and assets, plus party merges. Catalog/profile-database replay runs outside the UI thread and shows a determinate progress dialog whose stages correspond to completed validation, database replay, managed-state restoration, integrity verification, and interface refresh work. Restoring an asset also restores its app-managed file; external reference files remain untouched.</li>
          ...
          <li><strong>Protected financial history</strong>: Undo and Redo replay only the database records, settings, and managed files changed by the original action. Posted accounting entries, issued documents, royalty records, and later unrelated financial commands remain append-only and untouched; correct them through their dedicated void, credit, or reversal workflow.</li>
        </ul>
        <p>Snapshots capture the profile database and related managed state where supported, giving heavier workflows a safer recovery path than a simple session-only undo stack. Snapshots for SQLCipher profiles retain the same at-rest encryption and are never written as plaintext databases. While catalog/profile-database Undo or Redo is running, wait for its progress dialog to finish: starting a second replay is blocked, closing the app is blocked, and the progress bar reaches completion only after the refreshed interface reflects the committed result. If replay fails, the dialog closes with an actionable error and the same history action remains available to retry after the underlying problem is corrected. Cleanup previews exactly which artifacts are eligible, protects anything still required by undo, redo, snapshot restore, backup restore, or session restore, and leaves protected items untouched. Manual snapshots, manual database backups, and protected restore points stay protected by default, while automatic cleanup focuses on safe auto-generated artifacts only. If Diagnostics reports missing or inconsistent history artifacts, repair those issues first before trimming storage.</p>
        """,
    ),
    HelpChapter(
    ...
    HelpChapter(
        ...
        <ul>
          ...
          <li><strong>macOS credential access after replacement</strong>: an ad-hoc-signed replacement may no longer be recognized as the build that created existing Keychain entries. If remembered database passwords or SoundCloud credentials fail after an update, open <strong>Settings &gt; Application Settings &gt; General &gt; Security</strong> and manually choose <strong>Reset Stored Credentials…</strong>. The reset is never automatic, affects only generic-password entries under <code>isrc-catalog-manager.database</code> and <code>isrc-catalog-manager.soundcloud</code>, and requires confirmation with <strong>Cancel</strong> as the default. Restart afterward, re-enter existing database encryption passwords, and reconnect SoundCloud. See <strong>Application Settings</strong> for the full scope and consequences.</li>
        </ul>
    ...
    HelpChapter(
        ...
        <ul>
          ...
          <li><strong>Encrypted profile safety</strong>: if another encrypted profile exists in the app profile folder but has not been unlocked during the current session, Storage Admin still opens but marks its reference state as unknown. Managed files that might belong to that locked profile are warning-protected until you open that profile and refresh Storage Admin.</li>
