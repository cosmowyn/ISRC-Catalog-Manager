...
_RELEASE_NOTE_MARKDOWN_IMAGE_RE = re.compile(
    r"!\[[^\]]*]\((?P<url><[^>]+>|[^\s)]+)(?:\s+['\"][^'\"]*['\"])?\)"
)
_RELEASE_NOTE_HTML_IMAGE_RE = re.compile(
    r"<img\b[^>]*\bsrc\s*=\s*(?P<quote>['\"])(?P<url>.*?)(?P=quote)",
    re.IGNORECASE,
)
_RELEASE_NOTE_IMAGE_TIMEOUT_SECONDS = 3.0
_RELEASE_NOTE_IMAGE_MAX_BYTES = 512 * 1024
_RELEASE_NOTE_IMAGE_MAX_COUNT = 8
_RELEASE_NOTE_IMAGE_MAX_SIZE = QSize(960, 320)
...
def _iter_release_note_image_urls(markdown: str) -> list[str]:
    urls: list[str] = []
    seen: set[str] = set()
    text = str(markdown or "")
    for pattern in (_RELEASE_NOTE_MARKDOWN_IMAGE_RE, _RELEASE_NOTE_HTML_IMAGE_RE):
        for match in pattern.finditer(text):
            raw_url = html.unescape(str(match.group("url") or "")).strip()
            ...
            seen.add(raw_url)
            urls.append(raw_url)
            if len(urls) >= _RELEASE_NOTE_IMAGE_MAX_COUNT:
                return urls
    return urls


def _is_hydratable_release_note_image_url(url: str) -> bool:
    parsed = urlparse(str(url or "").strip())
    return parsed.scheme == "https" and bool(parsed.netloc)


def _fetch_release_note_image_bytes(url: str) -> tuple[str, bytes] | None:
    request = urllib.request.Request(
        url,
        headers={
            "Accept": "image/svg+xml, image/png, image/jpeg, image/gif, image/webp, */*",
            "User-Agent": "ISRC-Catalog-Manager-Release-Notes",
        },
    )
    try:
        with urllib.request.urlopen(
            request,
            timeout=_RELEASE_NOTE_IMAGE_TIMEOUT_SECONDS,
        ) as response:
            data = bytes(response.read(_RELEASE_NOTE_IMAGE_MAX_BYTES + 1))
            content_type = str(response.headers.get("content-type") or "")
    except OSError, TimeoutError, ValueError, socket.timeout, urllib.error.URLError:
        return None
    if len(data) > _RELEASE_NOTE_IMAGE_MAX_BYTES:
        return None
    return content_type, data


def _svg_release_note_image(data: bytes) -> QImage | None:
    ...
    if default_size.isValid() and default_size.width() > 0 and default_size.height() > 0:
        ...
        size = QSize(320, 80)
    if (
        size.width() > _RELEASE_NOTE_IMAGE_MAX_SIZE.width()
        or size.height() > _RELEASE_NOTE_IMAGE_MAX_SIZE.height()
    ):
        size.scale(_RELEASE_NOTE_IMAGE_MAX_SIZE, Qt.KeepAspectRatio)
...
def _release_note_image_from_bytes(content_type: str, data: bytes) -> QImage | None:
    clean_content_type = str(content_type or "").lower()
    clean_data = data.lstrip()
    if "svg" in clean_content_type or clean_data.startswith((b"<svg", b"<?xml")):
        return _svg_release_note_image(data)
    image = QImage.fromData(data)
    if image.isNull():
        return None
    if (
        image.width() > _RELEASE_NOTE_IMAGE_MAX_SIZE.width()
        or image.height() > _RELEASE_NOTE_IMAGE_MAX_SIZE.height()
    ):
        image = image.scaled(
            _RELEASE_NOTE_IMAGE_MAX_SIZE,
            Qt.KeepAspectRatio,
