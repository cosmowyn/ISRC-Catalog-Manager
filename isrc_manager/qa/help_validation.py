...
class HelpCoverageReport:
    ...
    chapter_screenshot_required_count: int
    chapter_screenshot_exempt_count: int
    chapter_screenshot_required_ids: list[str]
    chapter_screenshot_exempt_ids: list[str]
...
def validate_help_coverage(
    ...
    *,
    ...
    required_chapter_screenshot_references = tuple(
        iter_help_chapter_screenshot_references(chapters)
    )
    required_chapter_screenshot_ids = [
        reference.filename.removeprefix("chapter_").removesuffix(".png")
        for reference in required_chapter_screenshot_references
    ]
    exempt_chapter_screenshot_ids = [
        chapter.chapter_id
        for chapter in chapters
        if chapter.chapter_id in HELP_CHAPTER_SCREENSHOT_EXEMPT_IDS
    ]
    ...
    for reference in required_chapter_screenshot_references:
        ...
        if path.exists() and reference.filename in html_text and reference.caption in html_text:
            ...
            add_finding(
                ...
                expected=(
                    "Every non-exempt Help chapter embeds its relevant current UI "
                    "screenshot generated by the Help documentation qualification workflow."
                ),
    ...
    return HelpCoverageReport(
        ...
        screenshot_count=chapter_screenshot_count,
        chapter_screenshot_count=chapter_screenshot_count,
        chapter_screenshot_required_count=len(required_chapter_screenshot_references),
        chapter_screenshot_exempt_count=len(exempt_chapter_screenshot_ids),
        chapter_screenshot_required_ids=required_chapter_screenshot_ids,
        chapter_screenshot_exempt_ids=exempt_chapter_screenshot_ids,
