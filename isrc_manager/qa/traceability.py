...
def default_traceability_entries() -> list[TraceabilityEntry]:
    ...
    return [
        ...
        TraceabilityEntry(
            "UI-PQ-CAT-001",
            "catalog",
            "Catalog table, track creation, guarded track editing entry points, album operations.",
            "catalog table/actions",
            "Functional Workflow Qualification",
            "automated",
            "QA profile open.",
            "UI-populated Add Track and Edit Track forms.",
            "Create a track through Add Track UI, open Edit Track, verify manual ISRC recovery is available, save an edit, refresh catalog UI, and assert row and database state.",
            "Track exists in database, manual ISRC recovery is reachable, edited values persist, screenshot baseline comparison passes, and the row is visible to the catalog workflow.",
        ...
        TraceabilityEntry(
            "UI-PQ-MEDIA-001",
            "media_audio",
            "Media player, audio attachment, Image Preview export, conversion, and derivative/export ledger.",
            "media player, Image Preview, and audio actions",
            "Performance/Responsiveness Qualification",
            "automated",
            "QA profile open.",
            "Synthetic local WAV and PNG fixtures plus a deterministic no-ffmpeg conversion boundary.",
            "Attach audio through TrackService, render the bulk attach review dialog, route the Media Player command to the selected track, open the real Image Preview with a deterministic PNG, export it through the native picker boundary and file-history action, verify exact output bytes, export a managed lossy derivative through the controlled conversion service, and inspect the derivative ledger.",
            "Attached audio is fetchable, media player command targets the QA track, Image Preview writes an exact byte-for-byte copy through file.export_image_preview without an export error, derivative export writes one completed batch/item, and all governed surfaces are captured.",
            "Tracks media columns, image preview export file, HistoryEntries, TrackAudioWaveformCache, DerivativeExportBatches, TrackAudioDerivatives",
            "TrackService, media player controller, ImagePreviewDialog, App file-history routing, BulkAudioAttachDialog, ManagedDerivativeExportCoordinator, DerivativeLedgerService, VisualQualificationService",
            "evidence.json, image_preview_exports/*.png, business_workflow_manifest.json",
            "Audio attachment fails, media player target is wrong, Image Preview does not open, image bytes or hashes differ, file-history routing is absent or reports an error, derivative conversion/export fails, ledger rows are absent, or a governed UI surface is not captured.",
        ),
        TraceabilityEntry(
            "UI-PQ-ASSET-001",
            "assets_deliverables",
