...
class GS1TemplateVerificationService:
        ...
        try:
            ...
            return GS1TemplateProfile(
                ...
                field_options={
                    ...
                    for field_name, values in merged_field_options.items()
                    if values
                },
            ...
            for row in sheet.iter_rows(
                ...
                for value in row:
                    ...
                    if normalized and any(
                        keyword in normalized for keyword in self.KEYWORD_MARKERS
                    ):
                        markers.append(text)
        return markers

    def _scan_sheet_candidates(
        self, sheet, workbook_markers: list[str]
    ) -> list[GS1TemplateCandidate]:
        candidates: list[GS1TemplateCandidate] = []
    ...
    def _sheet_name_priority(self, sheet_name: str) -> float:
        ...
        if "{" in sheet_name or "}" in sheet_name:
            return -1.0
        if any(
            keyword in normalized for keyword in ("contract", "upload", "product", "gtin", "gs1")
        ):
            return 1.25
    ...
    def _extract_field_options(
        ...
        for field_name, values in self._validation_options_from_sheet_xml(
            workbook_path, workbook, column_map, sheet_name
        ).items():
            if values:
        ...
        for field_name in self.SETTINGS_OPTION_FIELDS:
            ...
            if column_index is None:
                continue
            existing_values = self._collect_existing_column_values(
                worksheet, column_index, start_row=header_row + 1
            )
            if existing_values:
    ...
    def _validation_options_from_sheet_xml(
        ...
        column_to_field = {
            column_index: field_name for field_name, column_index in column_map.items()
        }
    ...
    def _read_validation_entries(
        self, workbook_path: Path, worksheet_xml_path: str
    ) -> list[tuple[str, str]]:
        with ZipFile(workbook_path) as archive:
        ...
        for validation in worksheet_root.findall(".//x14:dataValidation", self._XML_NS):
            ...
            sqref = (
                str(sqref_element.text or "").strip()
                if sqref_element is not None
                else str(validation.attrib.get("sqref") or "").strip()
            )
    ...
    def _resolve_validation_formula_values(self, workbook, formula: str) -> list[str]:
        ...
        if clean_formula.startswith('"') and clean_formula.endswith('"'):
            return self._dedupe_preserve_order(
                part.strip() for part in clean_formula.strip('"').split(",")
            )
    ...
    @staticmethod
    def _collect_existing_column_values(
        worksheet, column_index: int, *, start_row: int = 2, max_rows: int = 512
    ) -> list[str]:
        values: list[str] = []
